...
function appendArray(currentValue, sampleType) {
    return __spreadArray(__spreadArray([], __read((currentValue ? currentValue : [])), false), __read(((0, type_utils_1.isArray)(sampleType) ? sampleType : [sampleType])), false);
}
function updateThreshold(currentValue, percent) {
    return ((0, type_utils_1.isUndefined)(currentValue) ? 1 : currentValue) * percent;
}
...
function sampleOnProperty(prop, percent) {
    return simpleHash(prop) % 100 < (0, number_utils_1.clampToRange)(percent * 100, 0, 100);
}
